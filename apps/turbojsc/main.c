...
static FILE *outfile;
static const char *c_ident_prefix = "turbojsc_";
static int strip;
...
#define PROG_NAME "turbojsc"

void help(void)
{
    printf("TurboJS Compiler version %s\n"
...
static int64_t parse_limit(const char *arg) {
    ...
    if (p == arg) {
        fprintf(stderr, "turbojsc: invalid limit: %s\n", arg);
        return -1;
    }
    ...
    if (*p) {
        ...
        default:
            fprintf(stderr, "turbojsc: invalid limit: %s, unrecognized suffix, only k,m,g are allowed\n", arg);
            return -1;
        }
        if (*p) {
            fprintf(stderr, "turbojsc: invalid limit: %s, only one suffix allowed\n", arg);
            return -1;
        }
    }
...
{
    if (optind >= argc) {
        fprintf(stderr, "turbojsc: missing file for -%c\n", opt);
        exit(1);
    }
}
...
int main(int argc, char **argv)
{
    ...
    if (output_type != OUTPUT_RAW) {
        fprintf(fo, "/* File generated automatically by the TurboJS compiler. */\n"
                "\n"
                );
    }
