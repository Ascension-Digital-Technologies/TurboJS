...
static void build_sum_graph(TurboJSSSAGraph *g)
{
    ...
    g->values[3]=(TurboJSSSAValue){.id=3,.block=0,.opcode=TURBOJS_SSA_CONSTANT_I64,.type=TURBOJS_SSA_TYPE_INT64,.immediate=0};
    g->values[4]=(TurboJSSSAValue){.id=4,.block=1,.opcode=TURBOJS_SSA_PHI,.type=TURBOJS_SSA_TYPE_INT32,.left=2,.right=11};
    g->values[5]=(TurboJSSSAValue){.id=5,.block=1,.opcode=TURBOJS_SSA_PHI,.type=TURBOJS_SSA_TYPE_INT64,.left=3,.right=9};
    ...
    g->values[8]=(TurboJSSSAValue){.id=8,.block=2,.opcode=TURBOJS_SSA_ELEMENT_LOAD,.type=TURBOJS_SSA_TYPE_INT64,.left=0,.right=4,.element_kind=TURBOJS_ELEMENT_KIND_PACKED_I64,.element_generation=7,.element_length_value=1};
    g->values[9]=(TurboJSSSAValue){.id=9,.block=2,.opcode=TURBOJS_SSA_ADD_I64,.type=TURBOJS_SSA_TYPE_INT64,.left=5,.right=8};
    ...
    g->values[13]=(TurboJSSSAValue){.id=13,.block=3,.opcode=TURBOJS_SSA_RETURN,.type=TURBOJS_SSA_TYPE_INT64,.left=5};
}

int main(void)
{
    TurboJSSSAGraph g; TurboJSSSAOptimizationStats os; TurboJSRegionNativeStats st={0};
    TurboJSRegionNativeFunction *native=NULL; TurboJSIRDiagnostic d={0}; TurboJSRegionValue result=0;
    TurboJSRegionValue args[2];
    TurboJSRegionValueOps ops={.guard_shape=guard_shape,.load_own_slot=load_slot,.to_i64=to_i64,.from_i64=from_i64};
    MockArray a={TURBOJS_ELEMENT_KIND_PACKED_I64,0,7,8,{1,2,3,4,5,6,7,8}};
    ...
    args[0]=(TurboJSRegionValue)(uintptr_t)&a; args[1]=8;
    ...
    CHECK(st.inline_element_loops==1); CHECK(st.inline_element_loop_length_hoists==1);
    CHECK(st.inline_element_loop_base_hoists==1); CHECK(st.inline_element_loop_bounds_eliminated==1);
    CHECK(st.inline_element_loop_unroll_factor==4); CHECK(st.inline_element_loop_accumulators==2);
    CHECK(TurboJS_RegionNativeInvokeValues(native,args,2,&ops,NULL,&result)==TURBOJS_IR_OK); CHECK(result==36);
    args[1]=5; CHECK(TurboJS_RegionNativeInvokeValues(native,args,2,&ops,NULL,&result)==TURBOJS_IR_OK); CHECK(result==15);
    args[1]=9; CHECK(TurboJS_RegionNativeInvokeValues(native,args,2,&ops,NULL,&result)==TURBOJS_IR_BAILOUT);
    a.generation=8; args[1]=8; CHECK(TurboJS_RegionNativeInvokeValues(native,args,2,&ops,NULL,&result)==TURBOJS_IR_BAILOUT);
    TurboJS_RegionNativeFunctionDestroy(native); TurboJS_SSAGraphDestroy(&g);
    puts("native element loop test passed");
    return 0;
}
