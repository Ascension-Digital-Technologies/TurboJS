...
{
    TurboJSOptimizationConfig config;
    config.baseline_threshold = 64;
    config.optimizing_threshold = 384;
    config.osr_threshold = 96;
    ...
    config.enable_optimizing_jit = config.enable_jit;
    config.enable_osr = config.enable_jit;
#else
                                                     ...
                                                     uint64_t function_id)
{
    ...
    if (profiler->config.enable_jit &&
        profiler->config.enable_optimizing_jit &&
...
void TurboJS_SetRuntimeOptimizationConfig(
    JSRuntime *rt, const TurboJSOptimizationConfig *config)
{
    turbojs_internal_set_optimization_config(rt, config);
}

TurboJSOptimizationConfig TurboJS_GetRuntimeOptimizationConfig(const JSRuntime *rt)
{
    return turbojs_internal_get_optimization_config(rt);
}
...
void TurboJS_ResetRuntimeJITStats(JSRuntime *rt)
{
    turbojs_internal_reset_jit_stats(rt);
}
