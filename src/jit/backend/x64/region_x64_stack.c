...
case TURBOJS_SSA_ADD_I64:case TURBOJS_SSA_SUB_I64:case TURBOJS_SSA_MUL_I64:if(!load(&b,0,v->left)||!load(&b,1,v->right))goto oom;if(v->opcode==TURBOJS_SSA_ADD_I64){if(!bin(&b,0x01,0,1))goto oom;}else if(v->opcode==TURBOJS_SSA_SUB_I64){if(!bin(&b,0x29,0,1))goto oom;}else if(!imul(&b,0,1))goto oom;if(!store(&b,v->id,0))goto oom;break;
case TURBOJS_SSA_LESS_THAN_I64:if(!load(&b,0,v->left)||!load(&b,1,v->right)||!bin(&b,0x39,0,1)||!e8(&b,0x0F)||!e8(&b,0x9C)||!e8(&b,0xC0)||!e8(&b,0x48)||!e8(&b,0x0F)||!e8(&b,0xB6)||!e8(&b,0xC0)||!store(&b,v->id,0))goto oom;break;
