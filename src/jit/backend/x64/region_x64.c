...
static int emit_less_than_cmp(Buf*b,const TurboJSSSAGraph*g,uint32_t left_id,Loc left,uint32_t right_id,Loc right){int32_t imm;if(!readloc(b,left,11))return 0;if(value_i32_constant(g,right_id,&imm))return binri32(b,11,7,imm);return op_right(b,0x39,0x3B,11,right);}
...
case TURBOJS_SSA_ADD_I64:case TURBOJS_SSA_SUB_I64:case TURBOJS_SSA_MUL_I64:{int32_t imm;if(!readloc(&b,left,11))goto oom;if(value_i32_constant(g,v->right,&imm)){if(v->opcode==TURBOJS_SSA_ADD_I64){if(!binri32(&b,11,0,imm))goto oom;}else if(v->opcode==TURBOJS_SSA_SUB_I64){if(!binri32(&b,11,5,imm))goto oom;}else if(!imulri32(&b,11,imm))goto oom;}else if(v->opcode==TURBOJS_SSA_ADD_I64){if(!op_right(&b,0x01,0x03,11,right))goto oom;}else if(v->opcode==TURBOJS_SSA_SUB_I64){if(!op_right(&b,0x29,0x2B,11,right))goto oom;}else if(!mul_right(&b,11,right))goto oom;if(!writeloc(&b,dst,11))goto oom;break;}
case TURBOJS_SSA_LESS_THAN_I64:if(value_has_only_branch_use(g,v->id))break;if(!emit_less_than_cmp(&b,g,v->left,left,v->right,right)||!rex(&b,0,0,0,11)||!e8(&b,0x0F)||!e8(&b,0x9C)||!e8(&b,0xC3)||!rex(&b,1,11,0,11)||!e8(&b,0x0F)||!e8(&b,0xB6)||!e8(&b,0xDB)||!writeloc(&b,dst,11))goto oom;break;
