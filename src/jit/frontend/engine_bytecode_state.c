...
TurboJSIRStatus TurboJS_EngineBytecodeBuildRegionStateGraph(
    ...
    size_t nslots,bi;uint32_t *phi_map=NULL,*ins_value=NULL,*queue=NULL;uint8_t*queued=NULL;
    uint32_t *visits=NULL;size_t qh=0,qt=0,qcap=0;TurboJSIRStatus st;
    ...
    qcap=g->plan.block_count?g->plan.block_count*2u:2u;
    queue=(uint32_t*)malloc(qcap*sizeof(uint32_t));queued=(uint8_t*)calloc(g->plan.block_count,1);
    visits=(uint32_t*)calloc(g->plan.block_count,sizeof(uint32_t));
    if(!g->blocks||(!g->entry_locals&&g->local_count)||(!g->exit_locals&&g->local_count)||!g->entry_stack||!g->exit_stack||!phi_map||!ins_value||!queue||!queued||!visits){st=fail(d,TURBOJS_IR_OUT_OF_MEMORY,0,"unable to allocate region state graph");goto done;}
    ...
    while(qh<qt){uint32_t bidx=queue[qh++],depth=g->plan.cfg.blocks[bidx].entry_stack_depth,ii;
        uint32_t *locals=NULL,*stack=NULL;int changed=0;queued[bidx]=0;
        if(++visits[bidx] > 128u){st=fail(d,TURBOJS_IR_UNSUPPORTED,g->plan.cfg.blocks[bidx].start_offset,"region state did not converge");goto done;}
        locals=(uint32_t*)malloc((g->local_count?g->local_count:1u)*sizeof(uint32_t));
        stack=(uint32_t*)malloc((g->stack_stride?g->stack_stride:1u)*sizeof(uint32_t));
        if(!locals||!stack){free(locals);free(stack);st=fail(d,TURBOJS_IR_OUT_OF_MEMORY,g->plan.cfg.blocks[bidx].start_offset,"unable to allocate region block state");goto done;}
        ...
        for(ii=0;ii<g->plan.cfg.blocks[bidx].instruction_count;ii++){
            ...
            if(op==OP_object){
                if(ins_value[ino]==TURBOJS_REGION_NO_VALUE)
                    ins_value[ino]=value_new(g,TURBOJS_REGION_VALUE_HELPER,bidx,off,op,TURBOJS_REGION_NO_VALUE,TURBOJS_REGION_NO_VALUE,0,1);
                if(ins_value[ino]==TURBOJS_REGION_NO_VALUE){st=fail(d,TURBOJS_IR_OUT_OF_MEMORY,off,"unable to allocate virtual object value");goto done;}
                stack[depth++]=ins_value[ino];continue;
            }
            if(op==OP_define_field){
                if(depth<2){st=fail(d,TURBOJS_IR_INVALID_OPCODE,off,"region field definition underflow");goto done;}
                if(ins_value[ino]==TURBOJS_REGION_NO_VALUE){
                    ins_value[ino]=value_new(g,TURBOJS_REGION_VALUE_HELPER,bidx,off,op,stack[depth-2],stack[depth-1],0,1);
                ...
                if(ins_value[ino]==TURBOJS_REGION_NO_VALUE){st=fail(d,TURBOJS_IR_OUT_OF_MEMORY,off,"unable to allocate virtual field definition");goto done;}
                depth-=2;stack[depth++]=ins_value[ino];continue;
            }
            if(op==OP_get_field){
            ...
            if(pop>=0){if(depth<(uint32_t)pop){st=fail(d,TURBOJS_IR_INVALID_OPCODE,off,"region operation underflow");goto done;}if(pop>0)right=stack[depth-1];if(pop>1)left=stack[depth-2];depth-=(uint32_t)pop;
                if(push>0){uint8_t kind=(op==OP_push_i32||op==OP_push_0||op==OP_push_1||op==OP_push_2||op==OP_push_3||op==OP_push_true||op==OP_push_false||op==OP_undefined||op==OP_null)?TURBOJS_REGION_VALUE_CONSTANT:((op==OP_add||op==OP_sub||op==OP_mul||op==OP_lt||op==OP_and||op==OP_or||op==OP_xor||op==OP_shl||op==OP_sar||op==OP_shr)?TURBOJS_REGION_VALUE_OPERATION:TURBOJS_REGION_VALUE_HELPER);
        ...
        if(changed){uint32_t si;for(si=0;si<g->plan.cfg.blocks[bidx].successor_count;si++){uint32_t s=g->plan.cfg.blocks[bidx].successors[si],j;int sch=0;for(j=0;j<g->local_count;j++){uint32_t m=merge_slot(g,s,j,0,phi_map);if(eloc(g,s)[j]!=m){eloc(g,s)[j]=m;sch=1;}}for(j=0;j<g->plan.cfg.blocks[s].entry_stack_depth;j++){uint32_t m=merge_slot(g,s,j,1,phi_map);if(estk(g,s)[j]!=m){estk(g,s)[j]=m;sch=1;}}if(sch&&!queued[s]){
                    if(qt==qcap){size_t nc=qcap<16u?16u:qcap*2u;void*np=realloc(queue,nc*sizeof(*queue));if(!np){st=fail(d,TURBOJS_IR_OUT_OF_MEMORY,g->plan.cfg.blocks[s].start_offset,"unable to grow region worklist");goto done;}queue=(uint32_t*)np;qcap=nc;}
                    queue[qt++]=s;queued[s]=1;}}}
    }
...
done:free(phi_map);free(ins_value);free(queue);free(queued);free(visits);if(st!=TURBOJS_IR_OK)TurboJS_EngineBytecodeRegionStateGraphDestroy(g);return st;
}
