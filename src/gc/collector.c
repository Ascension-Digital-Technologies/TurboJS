/* Engine domain source: gc/gc_exceptions_properties.inc -> collector.
...
static void compute_jsstring_size(JSString *str, JSMemoryUsage_helper *hp)
{
    if (!str->atom_type) {  /* atoms are handled separately */
        double s_ref_count = JS_REF_COUNT(str);
...
void turbojs_internal_compute_memory_usage(JSRuntime *rt, JSMemoryUsage *s)
{
    struct list_head *el, *el1;
    int i;
    ...
    list_for_each(el, &rt->gc_obj_list) {
        ...
        if (JS_GC_TYPE(gp) == JS_GC_OBJ_TYPE_FUNCTION_BYTECODE) {
            ...
            continue;
        } else if (JS_GC_TYPE(gp) != JS_GC_OBJ_TYPE_JS_OBJECT) {
            continue;
        }
        p = (JSObject *)gp;
        sh = p->shape;
        ...
        case JS_CLASS_BYTECODE_FUNCTION: /* u.func */
            {
                ...
                if (var_refs) {
                    ...
                    for (i = 0; i < b->closure_var_count; i++) {
                        if (var_refs[i]) {
                            double ref_count = JS_REF_COUNT(var_refs[i]);
...
void turbojs_internal_dump_memory_usage(FILE *fp, const JSMemoryUsage *s, JSRuntime *rt)
{
    fprintf(fp, "TurboJS memory usage -- %s version, %d-bit, malloc limit: %"PRId64"\n\n",
